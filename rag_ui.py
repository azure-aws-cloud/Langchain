...
st.subheader("Upload raw reference materials and query them seamlessly using native LangChain LCEL.")
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        if "last_uploaded" not in st.session_state or st.session_state.last_uploaded != uploaded_file.name:
            with st.spinner("Analyzing text schema and generating vector index embeddings..."):
...
if st.session_state.retriever is None:
    ...
    if user_query := st.chat_input("Ask a question about the uploaded document context..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching vector index space & running generation model inference..."):
                try:
