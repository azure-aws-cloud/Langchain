...
def get_embeddings_model():
    """Initializes the standard Gemini embedding instance securely."""
...
def process_text_into_persistent_store(raw_text: str):
    """Chunks text content and indexes it into the local on-premise Chroma folder."""
    text_splitter = RecursiveCharacterTextSplitter(chunk_size=500, chunk_overlap=50)
    docs = text_splitter.create_documents([raw_text])

    embeddings = get_embeddings_model()

    # Initialize and save to local PC disk storage
...
# Check disk space on startup to see if an index already exists
if "retriever" not in st.session_state:
    if os.path.exists(DB_DIR) and len(os.listdir(DB_DIR)) > 0:
        try:
            embeddings = get_embeddings_model()
            # Direct on-premise directory load
            ...
            st.error(f"Failed to auto-load existing index on disk: {startup_err}")
    ...
    if uploaded_file is not None:
        if "last_uploaded" not in st.session_state or st.session_state.last_uploaded != uploaded_file.name:
            with st.spinner("Chunking text layout and updating disk index..."):
                ...
                # Append file vectors directly to disk persistence
                st.session_state.retriever = process_text_into_persistent_store(raw_context)
                st.session_state.last_uploaded = uploaded_file.name
                st.rerun()  # Refresh layout to reflect updated model status immediately

    st.markdown("---")
    st.subheader("⚙️ Maintenance Panel")

    # Safe database reset option
...
if st.session_state.retriever is None:
    ...
    # Render historical conversation log elements
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])

    # Handle incoming conversational inputs
    if user_query := st.chat_input("Ask a question anchored to your local disk databases..."):
        st.session_state.messages.append({"role": "user", "content": user_query})
        with st.chat_message("user"):
            st.markdown(user_query)

        # Execute target model search queries via underlying LCEL pipelines
